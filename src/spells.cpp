...
bool sleep_monsters1(int row, int col) {
    ...
    for (int y = row - 1; y <= row + 1; y++) {
        for (int x = col - 1; x <= col + 1; x++) {
            ...
            Monster_t *m_ptr = &monsters_list[cave[y][x].cptr];
...
bool clone_monster(int dir, int y, int x) {
    ...
    while (!finished) {
        ...
        if (dist > OBJ_BOLT_RANGE || c_ptr->fval >= MIN_CLOSED_SPACE) {
            ...
            monsters_list[c_ptr->cptr].csleep = 0;

            // monptr of 0 is safe here, since can't reach here from creatures
            return multiply_monster(y, x, (int) monsters_list[c_ptr->cptr].mptr, 0);
        }
    }
...
void teleport_away(int monptr, int dis) {
    ...
    Monster_t *m_ptr = &monsters_list[monptr];
