...
static void get_moves(int monsterID, int *mm) {
    ...
    int y = m_list[monsterID].fy - char_row;
    int x = m_list[monsterID].fx - char_col;
...
void creatureCastsSpell(Monster_t *m_ptr, int monsterID, int spellID, uint8_t level, vtype_t cdesc, vtype_t deathDescription) {
    ...
    switch (spellID) {
        ...
        case 20: // Breath Light
            ...
            msg_print(cdesc);
            breath(GF_LIGHTNING, char_row, char_col, (m_ptr->hp / 4), deathDescription, monsterID);
            break;
        case 21: // Breath Gas
            (void) strcat(cdesc, "breathes gas.");
            msg_print(cdesc);
            breath(GF_POISON_GAS, char_row, char_col, (m_ptr->hp / 3), deathDescription, monsterID);
            break;
        case 22: // Breath Acid
            (void) strcat(cdesc, "breathes acid.");
            msg_print(cdesc);
            breath(GF_ACID, char_row, char_col, (m_ptr->hp / 3), deathDescription, monsterID);
            break;
        case 23: // Breath Frost
            (void) strcat(cdesc, "breathes frost.");
            msg_print(cdesc);
            breath(GF_FROST, char_row, char_col, (m_ptr->hp / 3), deathDescription, monsterID);
            break;
        case 24: // Breath Fire
            (void) strcat(cdesc, "breathes fire.");
            msg_print(cdesc);
            breath(GF_FIRE, char_row, char_col, (m_ptr->hp / 3), deathDescription, monsterID);
            break;
        default:
            ...
            if (c_ptr->fval <= MAX_OPEN_SPACE && c_ptr->tptr == 0 && c_ptr->cptr != 1) {
                ...
                if (c_ptr->cptr > 1) {
                    ...
                    if ((creatures_list[creatureID].cmove & CM_EATS_OTHER)
                        ...
                        int result = place_monster(row, col, creatureID, false);
                        hack_monptr = -1;
